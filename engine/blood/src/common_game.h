...
#if defined(__AMIGA__) && !defined(_DEBUG)
#define dassert(...)
#else
...
{
	SetBitString((char *)pArray, nIndex);
}
static inline char TestBitString(unsigned char *pArray, int nIndex)
{
	return TestBitString((char *)pArray, nIndex);
}
inline void ClearBitString(unsigned char *pArray, int nIndex)
{
	ClearBitString((char *)pArray, nIndex);
}
#endif
