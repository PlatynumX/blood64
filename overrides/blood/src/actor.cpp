...
#ifndef EDUKE32
        int size = (tilesizx[pSprite->picnum]*pSprite->xrepeat*tilesizy[pSprite->picnum]*pSprite->yrepeat)>>1;
#else
...
#ifdef __AMIGA__
    if (!vbp && !v4 && !xvel[pSprite->index] && !yvel[pSprite->index] && !zvel[pSprite->index]) return;
#endif
...
//tstart = getusecticks();
...
#ifdef __AMIGA__
                        // do some pre-checks to avoid expensive CheckProximity calls
                        int oX = klabs(pSprite->x-pSprite2->x)>>4;
                        if (oX >= proxyDist) continue;

                        int oY = klabs(pSprite->y-pSprite2->y)>>4;
                        if (oY >= proxyDist) continue;
   ...
   //t3 += getusecticks()-tstart;tstart = getusecticks();
    ...
    }//t5 += getusecticks()-tstart;tstart = getusecticks();
    for (nSprite = headspritestat[kStatDude]; nSprite >= 0; nSprite = nextspritestat[nSprite])
    {
        spritetype *pSprite = &sprite[nSprite];
    ...
    aiProcessDudes();//t8 += getusecticks()-tstart;tstart = getusecticks();
    gFX.fxProcess();//t9 += getusecticks()-tstart;tstart = getusecticks();
}
...
void G_AddGameLight(int, int, int, int, int, int)
{
    /* Polymer dynamic lights are not used by Blood64's classic renderer. */
}

void actDoLight(int)
{
    /* No-op for the N64 classic software renderer. */
}
